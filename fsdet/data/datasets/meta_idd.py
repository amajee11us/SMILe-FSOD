...
def load_filtered_idd_instances(
    ...
    Args:
        ...
        split (str): one of "train", "val", "trainval"

    Args example:
        name: idd_trainval_all1_1shot
        ...
        for cls in classnames:
            with PathManager.open(os.path.join(split_dir,
                ...
                if isinstance(fileids_, str):
                    fileids_ = [fileids_]
                fileids_ = [fid.split('JPEGImages/')[-1].split('.jpg')[0] \
                                for fid in fileids_]
    ...
    if is_shots:
        for cls, fileids_ in fileids.items():
            ...
            # if use_more_base and cls in IDD_BASE_CATEGORIES[int(split_id)]:
...
def register_meta_idd(
    ...
    DatasetCatalog.register(
        name, lambda: load_filtered_idd_instances(

...
def load_filtered_idd_instances(
    ...
    is_joint = IDD_NOVEL_CATEGORIES[int(name.split('allbase')[-1])] if 'allbase' in name else []
    if 'allnovel' in name:
        include_classnames = IDD_NOVEL_CATEGORIES[int(name.split('allnovel')[-1][0])]
        print('Logging names for {} novel classes: '.format(len(include_classnames)), include_classnames)
    else:
        include_classnames = classnames
    if is_shots:
        fileids = {}
        split_dir = os.path.join("datasets", "iddsplit")
        if "seed" in name:
        ...
        for cls in include_classnames:
            with PathManager.open(
                ...
                if isinstance(fileids_, str):
                    fileids_ = [fileids_]
                fileids_ = [
                    fid.split("JPEGImages/")[-1].split(".jpg")[0] for fid in fileids_
                ]
                fileids[cls] = fileids_
    else:
        with PathManager.open(
            os.path.join(dirname, split + ".txt")
        ) as f:
...
def register_meta_idd(
    ...
    DatasetCatalog.register(
        name,
        lambda: load_filtered_idd_instances(

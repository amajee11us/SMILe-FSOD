...
def register_all_coco(root="datasets"):
    ...
    for shot in [1, 2, 3, 5, 10, 30]:
        for seed in range(10):
            seed = "" if seed == 0 else "_seed{}".format(seed)
            name = "coco_trainval_allnovel_{}shot{}".format(shot, seed)
            METASPLITS.append((name, "coco/trainval2014", ""))
...
def register_all_pascal_voc(root="datasets"):
    ...
    METASPLITS = [
        ...
        ("voc_2007_trainval_allbase1", "VOC2007", "trainval", "base_novel_1", 1),
        ("voc_2007_trainval_allbase2", "VOC2007", "trainval", "base_novel_2", 2),
        ("voc_2007_trainval_allbase3", "VOC2007", "trainval", "base_novel_3", 3),
        ("voc_2012_trainval_allbase1", "VOC2012", "trainval", "base_novel_1", 1),
        ("voc_2012_trainval_allbase2", "VOC2012", "trainval", "base_novel_2", 2),
        ("voc_2012_trainval_allbase3", "VOC2012", "trainval", "base_novel_3", 3),
    ...
    # ('voc_2007_trainval_all1_5shot','voc_2007_trainval_base1', 'voc_2012_trainval_base1')
    # register small meta datasets for fine-tuning stage
    for prefix in ["all", "novel"]:
    ...
    for sid in range(1, 4):
        for shot in [1, 2, 3, 5, 10]:
            for year in [2007, 2012]:
                for seed in range(100):
                    seed = "" if seed == 0 else "_seed{}".format(seed)
                    name = "voc_{}_trainval_allnovel{}_{}shot{}".format(
                        year, sid, shot, seed
                    )
                    dirname = "VOC{}".format(year)
                    img_file = "all_{}shot_split_{}_trainval".format(
                        shot, sid
                    )
                    keepclasses = (
                        "base_novel_{}".format(sid)
...
def register_all_idd_detection(root="datasets"):
    ...
    METASPLITS = [
        ...
        ("idd_2019_trainval_allbase1", "IDD_Detection", "train", "base_novel_1", 1),
        ("idd_2019_trainval_allbase2", "IDD_Detection", "train", "base_novel_2", 2),
        ("idd_2019_trainval_allbase3", "IDD_Detection", "train", "base_novel_3", 3),
    ...
    for sid in range(1, 4):
        for shot in [5, 10]:
            for year in [2019]:
                for seed in range(100):
                    seed = "" if seed == 0 else "_seed{}".format(seed)
                    name = "idd_{}_trainval_allnovel{}_{}shot{}".format(
                        year, sid, shot, seed
                    )
                    dirname = "IDD_Detection"
                    img_file = "all_{}shot_split_{}_trainval".format(

...
class ROIHeads(torch.nn.Module):
    ...
    def label_and_sample_proposals(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            iou, obj_idx = match_quality_matrix.max(dim=0)
            ...
            proposals_per_image.obj_idx = obj_idx[sampled_idxs]
...
class StandardROIHeads(ROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        box_prediction = self.box_predictor(box_features)
        if len(box_prediction) == 2:
            pred_class_logits, pred_proposal_deltas = box_prediction
            pred_feat_norm = None
        elif len(box_prediction) == 3:
            pred_class_logits, pred_proposal_deltas, pred_feat_norm = box_prediction
...
@ROI_HEADS_REGISTRY.register()
class DistillMarginROIHeads(StandardROIHeads):
    """
    It's "standard" in a sense that there is no ROI transform sharing
    or feature sharing between tasks.
    The cropped rois go to separate branches directly.
    ...
    def __init__(self, cfg, input_shape):
        super(DistillMarginROIHeads, self).__init__(cfg, input_shape)
        self.cfg = cfg
        self._init_box_head(cfg)
        self.device = torch.device(cfg.MODEL.DEVICE)
        init_way = torch.rand if RAND_MARGIN else torch.zeros
        self.margins = nn.parameter.Parameter(
            init_way(cfg.MODEL.ROI_HEADS.NUM_CLASSES+1).float(),requires_grad=True
        ).to(self.device)
    ...
    def _forward_box(self, features, proposals):
        ...
        # del box_features
        outputs = FastRCNNDistillOutputs(
            self.cfg,
            self.box2box_transform,
            pred_class_logits,
            outputs_teacher[0],
        ...
        # (0.5 + torch.sigmoid(self.margins[0])
        if self.training:
            return outputs.losses()
        else:
            pred_instances, _ = outputs.inference(
